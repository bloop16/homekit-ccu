...
* Legacy cleanup: the installer stops and removes what the old hap-homematic addon left behind (rc.d script, monit config, lighttpd config, WebUI button, `/usr/local/addons/hap-homematic`; its configuration directory stays as a backup). The monit service is now called `HomekitCCU` (was `HapHomeMatic`, which clashed with the leftover `monit_hap-homematic.cfg` and made `monit reload` fail), an outdated monit config is rewritten, and a failing `monit reload` is logged as one warning line without aborting the start
...
* HomeKit pairing kept when coming from hap-homematic: on the first start the bridge keys and accessory ids (`persist/AccessoryInfo.*.json`, `persist/IdentifierCache.*.json`) are taken over from `/usr/local/etc/config/addons/hap-homematic` if that bridge was paired and homekit-ccu has no pairing yet (once, logged). Restoring a backup in the configuration UI now also restores the pairing: the files wait in `persist.restore` and replace the running bridge's keys on the restart. Before, they were only copied when no `persist` folder existed, so after a new install Apple Home no longer recognised the bridge
