...
- Video doorbell: *Install ffmpeg* in its settings downloads the static build of the Homebridge project (ffmpeg-for-homebridge v2.2.2, FFmpeg 8.0; arm64, x64 and 32 bit arm), installs it only when its SHA-256 matches the one of the add-on into its own directory (kept on updates, not in the backup, removed on uninstall) and starts the video doorbells. Its snapshot can come from a URL or file (also uploaded, read again every 10 s) instead of the video, so a battery camera is not woken for every snapshot. It rings like the doorbell (states when they become active, rings closer than 3 s once) and on the same datapoints.

### Changed
- The video doorbell is an accessory of the bridge it is assigned to, like the doorbell, instead of an accessory with a setup code of its own. Assigned to a bridge it stayed out of it, so the paired bridge looked empty, and its own port was not opened in the firewall. A video doorbell added to Apple Home with its own code before has to be removed there; it appears in its bridge. Without ffmpeg it stays out of the bridge instead of showing an empty accessory.

### Fixed
- A video source that is neither a URL nor ffmpeg arguments starting with `-` (e.g. `re -f lavfi …`) was taken for a URL: ffmpeg looked for a file of that name and snapshot and stream failed. It is refused when saving, with the reason, and named in the log at the start.
- Live video stopped after about 30 s behind the restrictive firewall of the CCU: the viewer sends RTCP and talkback to a random UDP port, which the firewall drops, and the watchdog ended the stream. The return ports now come from 9950–9979, which the add-on opens while there is a video doorbell (and closes when there is none or the add-on is uninstalled).
- A video doorbell had no speaker without a talkback target and no microphone without audio; HAP requires both, and Apple Home then shows a doorbell but never asks it for a picture. It always has both now; audio and talkback still only stream when they are set.
- Live video with Opus audio broke off with packet times of 60 ms (Apple Watch, home hubs): the audio packets were limited to 188 bytes, too small for 60 ms at 24 kbit/s, and ffmpeg ended the whole stream. Their size now follows bit rate and packet time; a packet time Opus does not know takes 20 ms instead of ending the stream.
