...
* commander 15 (ESM-only, loaded via `require(esm)`, so Node.js 22.12 or newer is required), formidable 3, fakegato-history 0.6.7; moment replaced by lib/util/time.js; chalk replaced by `util.styleText`
* Cleanup: the unused old configuration server (`configservice.js`, routes, middleware, CCU scripts), `CHANGELOG.json`, the dead in-UI update flow (API methods `update`/`updateChangelog`), the Debmatic instructions (only OpenCCU is supported) and old build helpers are removed. The add-on `update_script` accepts only OpenCCU (`HM-RASPBERRYMATIC`) and checks Node.js 22+ before it copies anything
...
* Configuration UI: *HomeKit Instances → Settings* is now called *Publish devices*; when every bridge already publishes its devices the dialog says so and only offers *Close*
...
* binrpc 4.3 (no `binary`/`put` dependencies, fragmented TCP frames, no CPU spin on refused connects) and homematic-xmlrpc 2.0 (built-in XML writer, byte-identical output); mocha 12, c8 12; `npm audit`: 0 vulnerabilities
...
* Installer: a half-installed addon directory is removed before `npm i`; a failed npm install or a missing `index.js` stops the installation (no "Installation complete", no start); the Node.js check runs before the WebUI button is removed; BusyBox-compatible redirections. Config server: a request for a directory answers 404 instead of ending the config server, file read errors answer 500. The log goes to `/var/log/homekit-ccu.log` (or `-L <dir>`) when writable; the check never worked, so it always went to `/tmp`. The log is appended to, so the installer's lines survive the app start, and a log larger than 2 MB is moved to `homekit-ccu.log.1` at startup. update-check.cgi passes on only a version-shaped release tag
* Bridge PIN: a bridge created by the server itself (the default bridge of a fresh install) got a new random PIN on every start and could not be paired, because its PIN was stored as `pin` but read as `pincode`. Both keys are read now, and on startup every bridge is migrated to a stored `pincode` and `setupID` (logged once per bridge), so the configuration UI, the log and the published bridge show the same PIN. Generated PINs skip the trivial codes HomeKit rejects
* Moving from hap-homematic: the installer refuses to run while hap-homematic is still installed and says how to move (backup in hap-homematic, uninstall it, install homekit-ccu, restore the backup). Leftovers of an uninstalled hap-homematic are removed (monit config, lighttpd config, WebUI button; its configuration directory stays). The monit service is now called `HomekitCCU` (was `HapHomeMatic`, which clashed with the leftover `monit_hap-homematic.cfg` and made `monit reload` fail), an outdated monit config is rewritten, and a failing `monit reload` is logged as one warning line without aborting the start
* Configuration schema version: `config.json` gets `"configVersion": 2`. On the first start after the upgrade the session check of the configuration UI is turned on even if the old configuration had `"useCCCAuthentication": false` (hap-homematic wrote that as its local-mode default), with a warning in the log; the value is saved before the configuration server starts. An explicit `false` saved in the settings afterwards is kept. A bridge assignment stored as a one-element array is stored as the bridge id, and removing a bridge also moves devices assigned to it as an array to the default bridge
* HomeKit pairing kept when a backup is restored (from hap-homematic or homekit-ccu): the bridge keys and accessory ids (`persist/AccessoryInfo.*.json`, `persist/IdentifierCache.*.json`) wait in `persist.restore` and replace the running bridge's keys on the restart. Before, they were only copied when no `persist` folder existed, so after a new install Apple Home no longer recognised the bridge
