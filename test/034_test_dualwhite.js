...
describe('HomeKit-CCU Tests ' + testCase, () => {
  ...
  it('HAP-Homematic set LEVEL 25%', (done) => {
    ...
    chTar.setValue(25, () => {
      setTimeout(async () => {
        const value = await that.server._ccu.getValue('BidCos-RF.1357501497ABCD:1.LEVEL')
        try {
  ...
  it('HomeKit-CCU check WHITE 0.5', (done) => {
    ...
    const service = accessory.getService(Service.Lightbulb)
    assert.ok(service, 'Lightbulb Service not found')
    const ch = service.getCharacteristic(Characteristic.ColorTemperature)
  ...
  it('HAP-Homematic set WHITE 1%', (done) => {
    ...
    const service = accessory.getService(Service.Lightbulb)
    assert.ok(service, 'Lightbulb not found')
    const chTar = service.getCharacteristic(Characteristic.ColorTemperature)
    chTar.setValue(140, () => {
      setTimeout(async () => {
        const value = await that.server._ccu.getValue('BidCos-RF.1357501497ABCD:2.LEVEL')
        try {
          expect(value).to.be(1)
          done()
        } catch (e) {
