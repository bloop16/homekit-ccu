...
describe('HomeKit-CCU Tests ' + testCase, () => {
  ...
  it('HomeKit-CCU check Valve with 50% value', (done) => {
    ...
    const eveValve = new EveHomeKitValveTypes(that.server.gatoHomeBridge.hap)

    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(eveValve.Service.ValveService, 'TestDevice', false, '', true)
    assert.ok(service, 'ValveService Service not found')
    const ch = service.getCharacteristic(eveValve.Characteristic.CurrentValveState)
