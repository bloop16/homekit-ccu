...
describe('HomeKit-CCU video doorbell accessory', () => {
  ...
  // a key datapoint has no start value; presses closer than 3 s ring once (util/doorbellTrigger.js)
  it('adds the doorbell service and rings on a key press, three at once ring once', () => {
    ...
    accessory.registered[0].callback(true)
    accessory.registered[0].callback(true)
    // a ring is sent as event notification, also when the value does not change
    expect(events).to.eql([[0, 'event']])
  })
