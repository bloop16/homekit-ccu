const assert = require('assert')
const path = require('path')
const Logger = require(path.join(__dirname, '..', 'lib', 'logger.js'))
const Server = require(path.join(__dirname, '..', 'lib', 'Server.js'))
const Service = require('@homebridge/hap-nodejs').Service
const Characteristic = require('@homebridge/hap-nodejs').Characteristic
const { getCharacteristicValue } = require(path.join(__dirname, 'helpers', 'characteristicValue.js'))
const expect = require('expect.js')

const fs = require('fs')
const log = new Logger('HAP Test')
log.setDebugEnabled(false)

const testCase = 'HmIPW-DRBL4.json'

describe('HomeKit-CCU Tests ' + testCase, () => {
  const that = this

  before(async () => {
    log.debug('preparing tests')
    const datapath = path.join(__dirname, 'devices', testCase)
    const strData = fs.readFileSync(datapath).toString()
    if (strData) {
      that.data = JSON.parse(strData)

      that.server = new Server(log)

      await that.server.simulate(undefined, {
        config: {
          channels: Object.keys(that.data.ccu)
        },
        devices: that.data.devices,
        mappings: that.data.mappings,
        values: { // add dummy values so hazDatapoint will find this DP and the device will get HMIP Style battery checks

        }
      })
    } else {
      assert.ok(false, 'Unable to load Test data')
    }
  })

  after(() => {
    Object.keys(that.server._publishedAccessories).forEach(key => {
      const accessory = that.server._publishedAccessories[key]
      accessory.shutdown()
    })
  })

  it('HomeKit-CCU check test mode', (done) => {
    expect(that.server.isTestMode).to.be(true)
    done()
  })

  it('HomeKit-CCU check number of ccu devices', (done) => {
    expect(that.server._ccu.getCCUDevices().length).to.be(1)
    done()
  })

  it('HomeKit-CCU check number of mappend devices', (done) => {
    expect(Object.keys(that.server._publishedAccessories).length).to.be(1)
    done()
  })

  it('HomeKit-CCU check assigned services', (done) => {
    Object.keys(that.server._publishedAccessories).forEach(key => {
      const accessory = that.server._publishedAccessories[key]
      expect(accessory.serviceClass).to.be(that.data.ccu[accessory.address()])
    })
    done()
  })

  it('HomeKit-CCU check LEVEL 0%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL', 0)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering, 'TestDevice', false, '', true)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentPosition)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetPosition)
    assert.ok(chCur, 'TargetPosition Characteristics not found')
    try {
      expect(chCur.value).to.be(0)
      expect(chTar.value).to.be(0)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HomeKit-CCU check LEVEL 50%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL', 0.5)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentPosition)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetPosition)
    assert.ok(chCur, 'TargetPosition Characteristics not found')
    try {
      expect(chCur.value).to.be(50)
      expect(chTar.value).to.be(50)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HomeKit-CCU check LEVEL 100%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL', 1)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentPosition)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetPosition)
    assert.ok(chCur, 'TargetPosition Characteristics not found')
    try {
      expect(chCur.value).to.be(100)
      expect(chTar.value).to.be(100)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HomeKit-CCU check getter LEVEL 100%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentPosition)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    getCharacteristicValue(chCur, (context, value) => {
      try {
        expect(value).to.be(100)
        done()
      } catch (e) {
        done(e)
      }
    })
  })

  it('HomeKit-CCU check slat LEVEL 0%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL_2', 0)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentHorizontalTiltAngle)
    assert.ok(chCur, 'CurrentHorizontalTiltAngle Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
    assert.ok(chCur, 'TargetHorizontalTiltAngle Characteristics not found')
    try {
      expect(chCur.value).to.be(-90)
      expect(chTar.value).to.be(-90)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HomeKit-CCU check slat LEVEL 50%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL_2', 0.5)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentHorizontalTiltAngle)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
    assert.ok(chCur, 'TargetPosition Characteristics not found')
    try {
      expect(chCur.value).to.be(0)
      expect(chTar.value).to.be(0)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HomeKit-CCU check slat LEVEL 100%', (done) => {
    that.server._ccu.fireEvent('HmIPW.3445238272ABCD:2.LEVEL_2', 1)
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chCur = service.getCharacteristic(Characteristic.CurrentHorizontalTiltAngle)
    assert.ok(chCur, 'CurrentPosition Characteristics not found')
    const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
    assert.ok(chCur, 'TargetPosition Characteristics not found')
    try {
      expect(chCur.value).to.be(90)
      expect(chTar.value).to.be(90)
      done()
    } catch (e) {
      done(e)
    }
  })

  it('HAP-Homematic set LEVEL 50%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    accessory.delayOnSet = 10
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chTar = service.getCharacteristic(Characteristic.TargetPosition)
    chTar.setValue(50, () => {
      setTimeout(async () => {
        const value = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL')
        try {
          expect(value).to.be(0.5)
        } catch (e) {
          done(e)
        }
        const valueSlat = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL_2')
        try {
          expect(valueSlat).to.be(1.01) // Slats should be set to 1.01
          done()
        } catch (e) {
          done(e)
        }
      }, 20) // default delay is 750ms
    })
  })

  it('HAP-Homematic set LEVEL 100%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    const service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    const chTar = service.getCharacteristic(Characteristic.TargetPosition)
    chTar.setValue(100, () => {
      setTimeout(async () => {
        const value = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL')
        try {
          expect(value).to.be(1)
        } catch (e) {
          done(e)
        }
        const valueSlat = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL_2')
        try {
          expect(valueSlat).to.be(1.01) // Slats should be set to 1.01
          done()
        } catch (e) {
          done(e)
        }
      }, 20)
    })
  })

  it('HAP-Homematic set slats 0% Level should be 100%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    accessory.delayOnSet = 10
    const service = accessory.getService(Service.WindowCovering)
    const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
    chTar.setValue(-90, () => {
      setTimeout(async () => {
        const valueSlat = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL_2')
        try {
          expect(valueSlat).to.be(0)
        } catch (e) {
          done(e)
        }
        const value = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL')
        try {
          expect(value).to.be(1) // Normal Level should be 1 from prevoius test
          done()
        } catch (e) {
          done(e)
        }
      }, 20) // default delay is 750ms
    })
  })

  it('HAP-Homematic set slats 50% level 50%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    accessory.delayOnSet = 10
    const service = accessory.getService(Service.WindowCovering)
    const chLTar = service.getCharacteristic(Characteristic.TargetPosition)
    chLTar.setValue(50, () => {
      setTimeout(async () => {
        const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
        chTar.setValue(0, () => {
          setTimeout(async () => {
            const valueSlat = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL_2')
            try {
              expect(valueSlat).to.be(0.5)
            } catch (e) {
              done(e)
            }
            const value = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL')
            try {
              expect(value).to.be(0.5) // Normal Level should be 50%
              done()
            } catch (e) {
              done(e)
            }
          }, 20) // default delay is 750ms
        })
      }, 20)
    })
  })

  it('HAP-Homematic set slats 100% level will stay at 50%', (done) => {
    const accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    accessory.delayOnSet = 10
    const service = accessory.getService(Service.WindowCovering)
    const chTar = service.getCharacteristic(Characteristic.TargetHorizontalTiltAngle)
    chTar.setValue(90, () => {
      setTimeout(async () => {
        const valueSlat = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL_2')
        try {
          expect(valueSlat).to.be(1)
        } catch (e) {
          done(e)
        }
        const value = await that.server._ccu.getValue('HmIPW.3445238272ABCD:2.LEVEL')
        try {
          expect(value).to.be(0.5)
          done()
        } catch (e) {
          done(e)
        }
      }, 20) // default delay is 750ms
    })
  })
})
