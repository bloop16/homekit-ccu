...
module.exports = class FFMPEG {
  constructor (hap, name, videoConfig, log) {
    ...
    const numberOfStreams = videoConfig.maxStreams || 2
  ...
  handleSnapshotRequest (request, callback) {
    ...
    try {
      const ffmpeg = spawn(this.ffmpegRoot, (imageSource + ' -t 1 -s ' + resolution + ' -f image2 -').split(' '), { env: process.env })
  ...
  prepareStream (request, callback) {
    ...
    const currentAddress = getLocalAddress()
    const addressResp = {
    ...
    if (sessionID) {
      ...
      if (requestType === 'start') {
        ...
        if (sessionInfo) {
          ...
          try {
            const ffmpeg = spawn(this.ffmpegRoot, ffmpegCommand.split(' '), { env: process.env })
  ...
  createCameraControlService () {
    ...
    if (this.audio) {
      const microphoneService = new Service.Microphone()
