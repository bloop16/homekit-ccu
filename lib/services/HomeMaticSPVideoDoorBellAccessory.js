...
module.exports = class HomeMaticSPVideoDoorBellAccessory extends HomeMaticAccessory {
  ...
  addDoorbellService (Service, Characteristic) {
    ...
    // a key press (PRESS_SHORT, ...) has no start value, every event rings; the first value of a
    // state datapoint is its start value
    let startValue = !isActionDatapoint(doorBellSensor)
