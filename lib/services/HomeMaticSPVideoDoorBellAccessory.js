...
module.exports = class HomeMaticSPVideoDoorBellAccessory extends HomeMaticAccessory {
  ...
  probeCamera (settings) {
    ...
    if (!isValidSetupCode(this.pinCode())) {
      return fail(`pin code ${this.pinCode()} is invalid; use the form 123-45-678 and avoid trivial codes like 123-45-678 or 111-11-111`)
    }
  ...
  pinCode () {
    return this.getDeviceSettings('pin-code') || DEFAULT_PIN
  }
  ...
  static configurationItems () {
    return {
      ...
      'pin-code': { type: 'text', hint: '', label: 'PinCode', default: DEFAULT_PIN },
      ffmpegpath: { type: 'text', hint: '', label: 'Path to ffmpeg', default: '/usr/local/bin/ffmpeg' },
      vcodec: { type: 'text', hint: '\'copy\' when the camera delivers H.264 (recommended on the CCU itself); libx264 transcodes and needs a fast CPU', label: 'Video codec', default: 'libx264' },
      ...
      audio_return_target: { type: 'text', hint: 'ffmpeg output for talkback, empty disables two-way audio; raw options starting with - override codec, e.g. -codec:a pcm_mulaw -ar 8000 -f rtsp rtsp://cam/talk', label: 'Talkback target', default: '' }
    }
  }
