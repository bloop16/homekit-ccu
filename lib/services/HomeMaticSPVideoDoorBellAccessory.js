...
// a URL (rtsp://, http://, ...) or raw ffmpeg input arguments, which start with -
const VIDEO_SOURCE = /^(-|[a-z][a-z0-9+.-]*:\/\/)/i
const VIDEO_SOURCE_HINT = 'The video source must be a URL like rtsp://camera/stream or ffmpeg input arguments starting with - (e.g. -re -f lavfi -i testsrc)'
...
class HomeMaticSPVideoDoorBellAccessory extends HomeMaticAccessory {
  ...
  probeCamera (settings) {
    ...
    const invalid = HomeMaticSPVideoDoorBellAccessory.validateSettings({ video_source: this.getDeviceSettings('video_source') })
    if (invalid) {
      return fail(invalid)
    }
  ...
  // a video doorbell without ffmpeg would be an empty accessory in its bridge
  canBePublished () {
  ...
  /** the reason the settings cannot work (shown when saving), undefined when they can */
  static validateSettings (settings) {
    const source = String((settings && settings.video_source) || '').trim()
    return VIDEO_SOURCE.test(source) ? undefined : VIDEO_SOURCE_HINT
  }
...
module.exports = HomeMaticSPVideoDoorBellAccessory
