...
module.exports = class HomeMaticSPVideoDoorBellAccessory extends HomeMaticAccessory {
  createHomeKitAccessory () {
    ...
    this.homeKitAccessory = new Accessory(this._name, this._accessoryUUID, HAP.Categories.VIDEO_DOORBELL)
  ...
  publishServices (Service, Characteristic) {
    const self = this
    const ffmpegpath = path.join(this.getDeviceSettings('ffmpegpath') || '/bin')
    const camera = {
      ffmpegpath,
    ...
    if (fs.existsSync(ffmpegpath)) {
      ...
      const doorLock = new Service.LockMechanism(this._name)
      this.homeKitAccessory.addService(doorLock)

      const lockCurrentState = doorLock.getCharacteristic(Characteristic.LockCurrentState)
      ...
      const targetState = doorLock.getCharacteristic(Characteristic.LockTargetState)
  ...
  getPublishInfo () {
    ...
    return {
      ...
      category: HAP.Categories.VIDEO_DOORBELL,
      mdns: { multicast: true }
    }
  }
