...
/**
 * Remove secrets from an ffmpeg command line or log line: URL credentials, credentials in
 * query strings (user, username, pass, password, pwd) and SRTP keys.
 * @param {string} text
 * @returns {string}
 */
function redact (text) {
  return String(text)
    .replace(/\/\/[^\s/@]+@/g, '//***@')
    .replace(/(user|username|pass|password|pwd)=[^&\s]*/gi, '$1=***')
