...
const { bindUdpSocket, reserveUdpPortPair } = require(path.join(__dirname, 'udpPort.js'))
...
class StreamingDelegate {
  ...
  prepareStream (request, callback) {
    Promise.all([bindUdpSocket(request.addressVersion), reserveUdpPortPair(request.addressVersion)])
      .then(([videoReturnSocket, audioReturnPair]) => {
        videoReturnSocket.on('error', (err) => this.log.warn('[Camera %s] video return socket: %s', this.name, err.message))
        const session = {
  ...
  startStream (request, callback) {
    ...
    const stopWatchdog = startRtcpWatchdog(session.videoReturnSocket, timeout, () => {
      this.abortSession(sessionID, `no RTCP from viewer for ${timeout}s, stopping session ${sessionID}`)
    })
