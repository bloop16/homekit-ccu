...
// pairs handed out, until the stream releases them or HOLD_MS passed: the pair is free again
// when it is handed out (ffmpeg binds it), a second stream must not get it meanwhile
const HOLD_MS = 60000
const handedOut = new Map()

function isHandedOut (rtp, now) {
  const until = handedOut.get(rtp)
  if ((until !== undefined) && (until <= now)) {
    handedOut.delete(rtp)
  }
  return handedOut.has(rtp)
}

/** gives a pair of reserveUdpPortPair back when its stream ended */
function releaseUdpPortPair (rtp) {
  handedOut.delete(rtp)
}

/**
 * Reserve an even RTP port of the range whose odd neighbour (RTCP) is free as well, then
 * release both, so an ffmpeg SDP input can bind the pair; it is not handed out again until
 * releaseUdpPortPair (or HOLD_MS).
...
async function reserveUdpPortPair (addressVersion = 'ipv4', range = STREAM_PORTS) {
  for (const rtp of streamPorts(range).filter(port => (port % 2 === 0) && (port + 1 <= range.last))) {
    if (isHandedOut(rtp, Date.now())) {
      continue
    }
    // taken at once, so a concurrent call skips it while this one checks the ports
    handedOut.set(rtp, Date.now() + HOLD_MS)
...
module.exports = { STREAM_PORTS, streamPorts, bindReturnSocket, reserveUdpPortPair, releaseUdpPortPair, bindUdpSocket }
