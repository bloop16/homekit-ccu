...
// the smallest audio packet (as homebridge-camera-ffmpeg sends it); longer packet times need more
const AUDIO_PKT_SIZE = 188
// 12 bytes RTP header and 10 bytes SRTP tag, some to spare
const RTP_OVERHEAD = 26
// Opus has a variable bit rate: a packet may carry more than the average
const VBR_RESERVE = 1.5
// the frame durations libopus accepts, in ms
const OPUS_FRAMES = [2.5, 5, 10, 20, 40, 60]

/** the packet size of the audio: one packet of packet_time at max_bit_rate has to fit */
function audioPacketSize (audio) {
  const payload = Math.ceil(VBR_RESERVE * (audio.max_bit_rate || 24) * (audio.packet_time || 20) / 8)
  return Math.max(AUDIO_PKT_SIZE, RTP_OVERHEAD + payload)
}

/** the Opus frame duration for a packet time; one Opus does not know becomes 20 ms */
function opusFrameDuration (packetTime) {
  return OPUS_FRAMES.includes(packetTime) ? packetTime : 20
}
...
function audioEncoderArgs (audio) {
  const codec = audio.codec === AudioStreamingCodecType.AAC_ELD
    ...
    : ['-codec:a', 'libopus', '-application', 'lowdelay', '-frame_duration', String(opusFrameDuration(audio.packet_time))]
  return [
    ...codec,
    '-flags', '+global_header',
...
function buildStreamArgs (settings, session, request) {
  ...
  if (request.audio) {
    ...
    args.push(...srtpOutput(request.audio.pt, session.audioSSRC, session.audioSRTP, session, session.audioPort, audioPacketSize(request.audio)))
  }
  return args
}
...
module.exports = { buildSnapshotArgs, buildStreamArgs, buildReturnAudioSdp, buildReturnAudioArgs }
