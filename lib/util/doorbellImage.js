...
class StillImage {
  ...
  /** @returns {Promise<Buffer>} JPEG of this size */
  async snapshot (width, height) {
    const key = width + 'x' + height
    if (!this.snapshots.has(key)) {
      const jpg = renderSnapshot(await this.image(), width, height)
      if (this.snapshots.size >= MAX_CACHED_SIZES) {
        this.snapshots.delete(this.snapshots.keys().next().value)
      }
      this.snapshots.set(key, jpg)
    }
    return this.snapshots.get(key)
  }
}

module.exports = { decodeImage, renderSnapshot, loadImage, StillImage }
