...
/**
 * The picture centered, as JPEG of the requested size: a device picture on white, a photo (an
 * image with photo: true) filling it, with black bars when the sides do not fit.
...
function renderSnapshot (image, width, height) {
  // a device picture (transparent PNG) is drawn small on white, a photo fills the snapshot
  const photo = Boolean(image && image.photo)
  const canvas = Buffer.alloc(width * height * 4, image ? (photo ? 0 : WHITE) : FALLBACK_GREY)
  if (image) {
    const scale = photo
      ? Math.min(width / image.width, height / image.height)
      : Math.min(FILL * width / image.width, FILL * height / image.height, MAX_SCALE)
...
async function loadImage (source) {
  const image = decodeImage(await readSource(source))
  // a URL or a file is a photo (e.g. of a camera), the CCU picture a transparent device picture
  return { ...image, photo: source.kind !== 'ccu' }
}

/** the content of a picture source, see loadImage */
async function readSource (source) {
  ...
  switch (source && source.kind) {
    case 'ccu':
      ...
      return readLimited(path.join(source.wwwRoot || '/www', value))
    case 'url':
...
/**
 * The snapshot of a doorbell: the first source that loads, otherwise a plain grey image, one
 * JPEG per size asked for. A picture that is replaced (the snapshot a camera stores behind a URL
 * or in a file) is read again when a snapshot is asked for after refreshSeconds, in the
 * background: the request gets the stored picture at once, a changed one is shown from then on.
 * Reading it again never asks the camera for a picture, it only reads what is stored.
 */
class StillImage {
  /**
   ...
   * @param {{refreshSeconds?: number, now?: function}} options refreshSeconds 0: read once
   */
  constructor (sources, log, name, { refreshSeconds = 0, now = Date.now } = {}) {
    this.sources = sources.filter(source => source && source.value)
    this.log = log
    this.name = name
    this.refreshMs = Math.max(0, Number(refreshSeconds) || 0) * 1000
    this.now = now
    this.snapshots = new Map()
  }

  /** the picture to show; starts reading it again in the background when it is due */
  image () {
    if (!this.first) {
      this.first = this.load(true)
      return this.first
    }
    const due = (this.refreshMs > 0) && (this.now() - this.loadedAt >= this.refreshMs)
    if (due && !this.reloading) {
      this.reloading = this.load(false).finally(() => { this.reloading = undefined })
    }
    return this.first.then(() => this.current)
  }

  // the first source that can be read; a changed content replaces the picture and its snapshots
  async load (first) {
    ...
    for (const source of this.sources) {
      try {
        ...
        if (hash !== this.hash) {
          this.current = { ...decodeImage(content), photo: source.kind !== 'ccu', hash }
          this.hash = hash
    ...
    if (first) {
      ...
      this.log.debug('[Doorbell %s] reading the picture again failed (%s), keeping the last one', this.name, reasons.join('; '))
    }
    return this.current
  }
  ...
  async snapshot (width, height) {
    ...
    // keyed by the content as well: a rendering of a picture replaced meanwhile is never kept for the new one
    const key = ((image && image.hash) || 'plain') + ':' + width + 'x' + height
    if (!this.snapshots.has(key)) {
      if (this.snapshots.size >= MAX_CACHED_SIZES) {
        this.snapshots.delete(this.snapshots.keys().next().value)
      }
      const rendering = renderSnapshotInWorker(image, width, height)
      const snapshots = this.snapshots
      rendering.catch(() => snapshots.delete(key))
      snapshots.set(key, rendering)
    }
