...
// a doorbell: the doorbell service (with a still image camera, so Apple Home shows a doorbell),
// or the button as programmable switch for automations
const DOORBELL = ['HomeMaticDoorBellAccessory', KEY]
...
// channels that offer only the services of their preference: a doorbell is no window or garage door
const ONLY_PREFERRED = new Set([
  'HmIP-DSD-PCB:MULTI_MODE_INPUT_TRANSMITTER',
  'HmIP-DBB:KEY_TRANSCEIVER',
  'HM-Sen-DB-PCB:KEY'
])
...
function orderedServicesForChannel (serviceTable, deviceType, channelType) {
  ...
  const deviceChosen = devicePreferences(channelType, deviceType)
    ...
    .filter(entry => entry !== undefined)
  if (ONLY_PREFERRED.has(deviceType + ':' + channelType) && (deviceChosen.length > 0)) {
    return deviceChosen
  }
