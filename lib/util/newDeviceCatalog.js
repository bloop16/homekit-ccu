...
const { channelNumberOf, isRemoteKeyType } = require(path.join(__dirname, 'remoteMapping.js'))
const { coveringAddress } = require(path.join(__dirname, 'combinedChannels.js'))
const { deviceIcons } = require(path.join(__dirname, 'deviceIcons.js'))
const { isGroupAddress } = require(path.join(__dirname, 'ccuGroups.js'))
...
function catalogChannel (device, channel, context) {
  const { serviceTable, rooms, functions, mappings } = context
  const entry = {
    id: channel.id,
  ...
  } else if (coveringAddress(mappings, channel) !== undefined) {
    // a button of the device's remote or an output of a combined switch accessory
    entry.coveredBy = coveringAddress(mappings, channel)
  }
 ...
 * groups the CCU groups { groupAddress: [member addresses] } (util/ccuGroups.js).
 *
 * A group device carries group: true and its members; a member carries its groups, and its
 * channels of the group's kind (e.g. the thermostat of a heating group member) carry byGroup: the
 * group sets them, so they are not preselected. Other channels (e.g. a window contact) stay free.
 */
function buildDeviceCatalog (compatibleDevices, mappings, { serviceTable, rooms, functions, icons = deviceIcons(), groups = {} } = {}) {
  const catalog = (compatibleDevices || [])
    .map(device => {
      const deviceChannels = device.channels || []
      const supported = deviceChannels.filter(channel => channel.isSuported === true)
      const context = { serviceTable, rooms, functions, mappings }
  ...
  return withGroups(catalog, groups || {})
}

/** marks the group devices, their members and the member channels the group sets */
function withGroups (catalog, groups) {
  const byAddress = new Map(catalog.map(device => [device.address, device]))
  catalog.filter(device => isGroupAddress(device.address)).forEach(group => {
    group.group = true
    group.members = (groups[group.address] || []).filter(address => byAddress.has(address) && (address !== group.address))
    group.members.forEach(address => {
      const member = byAddress.get(address)
      member.groups = (member.groups || []).concat([group.address])
      member.channels.forEach(channel => {
        if ((channel.byGroup === undefined) && (categoryOf(channel.services[0].serviceClazz) === group.category)) {
          channel.byGroup = group.address
          channel.preselect = false
        }
      })
    })
  })
