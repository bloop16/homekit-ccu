 ...
 * { text, room, showMapped, showSecondary }; text matches device name, type, address, room and
 * channel name; room is a room name, '' for all rooms.
 */
export function visibleEntries (device, entries, filter = {}) {
  const text = String(filter.text || '').trim().toLowerCase()
  if ((filter.room) && !(device.rooms || []).includes(filter.room)) {
    return []
  }
  ...
  const deviceMatches = has(device.name) || has(device.type) || has(device.address) || (device.rooms || []).some(has)
  if (deviceMatches) {
    return shown
  }
