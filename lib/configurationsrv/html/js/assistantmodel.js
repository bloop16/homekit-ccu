...
import { buildEntries, isMapped, mappingsFor, preselectedEntries, suggestNames } from './newdevicemodel.js'
...
export function proposePlan (layout, byRoom, rooms, bridges, options = {}) {
  ...
  const plan = { layout, bridges: [], roomBridge: {}, securityBridge: undefined }
...
/** the bridge key of a device: the security bridge for locks and alarms, else its room's */
...
export function initialSelection (items) {
  ...
  items
    ...
    .forEach(item => preselectedEntries(item.entries).forEach(entry => selected.add(entry.id)))
  return selected
}
 ...
 */
export function summarize (plan, items, rooms, selected) {
  ...
  items.forEach(item => {
    ...
    if (summary && (chosen.length > 0)) {
      summary.devices += 1
      summary.accessories += chosen.length
    }
  })
 ...
 * rooms) and a mapping per selected row with the key of its bridge. Rows without a bridge are
 * skipped.
 */
export function buildApplyPayload (plan, items, rooms, selected, choices) {
  const devices = []
  const used = new Set()
  items.forEach(item => {
    const key = bridgeOfDevice(plan, item, rooms)
    if (!key) return
    item.entries.filter(entry => selected.has(entry.id) && !isMapped(entry)).forEach(entry => {
      const choice = choices[entry.id]
      mappingsFor(entry, { ...choice, name: String(choice.name).trim(), bridge: key })
        .forEach(mapping => {
