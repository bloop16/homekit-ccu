...
import { channelNumbers, hasDefaultName, isMapped, isPerKeyChoice, preselectedEntries, REMOTE_SERVICE, serviceLabel } from './newdevicemodel.js'
import {
...
export class SetupAssistant extends Wizzard {
  ...
  renderDeviceItem (item) {
    ...
    if (isOpen) {
      ...
      free.filter(entry => this.showSecondary || !entry.secondary || this.selected.has(entry.id))
        .forEach(entry => body.append(this.renderEntry(item, entry)))
      box.append(body)
    }
    return box
  }
  ...
  renderPreview () {
    const summary = summarize(this.plan, this.candidates, this.rooms, this.selected)
    const box = $('<div>')
  ...
  async apply () {
    const payload = buildApplyPayload(this.plan, this.candidates, this.rooms, this.selected, this.choices)
    this.applyButton.setActive(false)
