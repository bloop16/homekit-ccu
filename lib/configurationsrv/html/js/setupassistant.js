...
import { channelNumbers, combinableEntries, combinedName, entriesSetByGroup, hasDefaultName, orderWithGroups, isMapped, isPerKeyChoice, preselectedEntries, REMOTE_SERVICE, serviceLabel } from './newdevicemodel.js'
import {
  NO_ROOM, MAX_ACCESSORIES, addBridge, bridgeDisplayName, bridgeNameProblem, removeBridge, bridgeOfDevice, buildApplyPayload, chooseBridge, completeChoices, proposedBridgeOfDevice,
...
export class SetupAssistant extends Wizzard {
  ...
  goNext () {
    ...
    if (this.step === 'bridges') {
      const problem = bridgeNameProblem(this.plan, this.application.getBridges())
      if (problem) {
        this.error((problem === 'missing') ? this.__('Every new bridge needs a name.') : this.__('Every bridge needs a name of its own.'))
        return
      }
    }
  ...
  renderBridges () {
    const box = $('<div>')
    box.append($('<p>').text(this.__('Rename, remove or add bridges and choose the bridge of every room. A room set to "not taken" stays out of HomeKit.')))
    ...
    this.plan.bridges.forEach((bridge, index) => {
      ...
      const remove = $('<button>').attr({ type: 'button', title: this.__('Remove'), 'aria-label': this.__('Remove') + ': ' + (bridge.id ? bridge.name : bridgeDisplayName(bridge.name)) })
        .addClass('btn btn-sm btn-outline-danger').append($('<i>').addClass('bi bi-trash'))
        .on('click', () => {
          removeBridge(this.plan, bridge.key)
          this.show('bridges')
        })
      if (bridge.id) {
    ...
    if (this.plan.bridges.length === 0) {
      bridges.append($('<div>').addClass('small text-body-secondary mb-2').text(this.__('No bridge in the plan, add one.')))
    }
    // a further bridge, e.g. a room of its own or another floor; it gets rooms in the table below
    const isFloor = this.plan.layout === 'floor'
    const name = $('<input>').attr({ type: 'text', id: 'sa_newBridge', placeholder: isFloor ? this.__('e.g. Basement') : this.__('Name of the new bridge'), 'aria-label': this.__('Name of the new bridge') })
      .addClass('form-control form-control-sm')
    const addNew = () => {
      const value = String(name.val()).trim()
      if (value !== '') {
        addBridge(this.plan, value, isFloor ? 'floor' : 'custom')
        this.show('bridges')
        $('#sa_newBridge').trigger('focus')
      }
    }
    name.on('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); addNew() } })
    const add = $('<button>').attr('type', 'button').addClass('btn btn-sm btn-outline-primary text-nowrap')
      .append($('<i>').addClass('bi bi-plus-lg me-1')).append(document.createTextNode(isFloor ? this.__('Add floor') : this.__('Add bridge')))
      .on('click', addNew)
  ...
  // the chosen rows of devices that go to a bridge
  chosenEntries () {
    return this.candidates
      .filter(item => bridgeOfDevice(this.plan, item, this.rooms) !== '')
      .flatMap(item => item.entries.filter(entry => this.selected.has(entry.id) && !isMapped(entry)))
  }
  ...
  renderDeviceList () {
    ...
    // the sections in the order of the bridges, devices left out last (their bridge can be chosen again)
    this.plan.bridges.map(bridge => bridge.key).concat(['']).filter(key => groups.has(key)).forEach(key => {
      const section = $('<section>').addClass('mb-3' + ((key === '') ? ' sa-not-taken' : ''))
      const icon = (key === '') ? 'bi-slash-circle' : 'bi-house-door'
      section.append($('<h3>').addClass('h6 sa-bridge-title').append($('<i>').addClass('bi me-1 ' + icon).attr('aria-hidden', 'true')).append(document.createTextNode(this.bridgeTitle(key))))
      // CCU groups above their members (a member on another bridge stays in that bridge's section)
      orderWithGroups(groups.get(key)).forEach(({ item, group }) => section.append(this.renderDeviceItem(item, group)))
  ...
  renderDeviceItem (item, group) {
    ...
    const box = $('<div>').addClass('nd-device border rounded mb-2' + (group ? ' nd-member' : ''))
    ...
    const check = $('<input>').attr({ type: 'checkbox', 'aria-label': device.name }).addClass('form-check-input m-0 flex-shrink-0')
      ...
      .on('click', (e) => {
        ...
        if (chosen.length > 0) {
          ...
          // a group sets its members: their thermostat (or switch ...) is not needed twice
          if (device.group === true) {
            entriesSetByGroup(this.candidates, device.address).forEach(entry => this.selected.delete(entry.id))
          }
        }
    ...
    const details = [device.type].concat(device.rooms || []).concat(device.functions || [])
    if (group) {
      // the name goes in as text: localize escapes the values it inserts
      details.unshift(this.__('member of %s', '\u0000').replace('\u0000', group.device.name))
    }
    title.append($('<div>').addClass('small text-body-secondary text-truncate').text(details.join(' · ')))
    head.append(title)
    if (device.group === true) {
      head.append($('<span>').addClass('badge rounded-pill text-bg-info flex-shrink-0').html(this.__('Group with %s devices', (device.members || []).length)))
    }
    if (chosen.length > 0) {
      head.append($('<span>').addClass('badge rounded-pill text-bg-primary flex-shrink-0').html(this.__('%s selected', chosen.length)))
    }
    head.append(this.renderBridgeChoice(item))
    head.on('click', () => {
  ...
  /** the bridge of a device; another choice moves the device into that bridge's section */
  renderBridgeChoice (item) {
    const id = 'sa_bridge_of_' + item.device.address.replace(/[^A-Za-z0-9]/g, '_')
    const current = bridgeOfDevice(this.plan, item, this.rooms)
    const select = $('<select>').attr({ id, 'aria-label': this.__('Bridge') }).addClass('form-select form-select-sm sa-bridge-select flex-shrink-0')
    this.plan.bridges.forEach(bridge => select.append($('<option>').val(bridge.key).text(bridge.id ? bridge.name : bridgeDisplayName(bridge.name))))
    select.append($('<option>').val('').text(this.__('not taken')))
    select.val(current)
      .on('click', (e) => e.stopPropagation())
      .on('change', () => {
        chooseBridge(this.plan, item, this.rooms, select.val())
        this.renderDeviceList()
      })
    const box = $('<div>').addClass('d-flex flex-column align-items-end flex-shrink-0').on('click', (e) => e.stopPropagation()).append(select)
    if (current !== proposedBridgeOfDevice(this.plan, item, this.rooms)) {
      box.append($('<span>').addClass('small text-body-secondary').text(this.__('differs from the room')))
    }
    return box
  }
  ...
  renderEntry (item, entry, combined = []) {
    ...
    if (entry.byGroup) {
      label.append($('<span>').addClass('badge text-bg-light border').text(this.__('set by the group')))
    }
    row.append(label)
    wrap.append(row)
  ...
  renderPairing () {
    ...
    box.append($('<p>').text(this.__('Create the rooms in Apple Home first (+, "Add Room"): while adding a bridge, Apple Home only offers existing rooms. Then add every new bridge: tap +, "Add Accessory", scan the code and choose the room shown here. Then publish the devices.')))
