...
import {
  REMOTE_SERVICE, buildEntries, channelNumbers, deviceState, hasDefaultName, isMapped, isPerKeyChoice,
...
export class NewDeviceWizzard extends Wizzard {
  constructor (application) {
    ...
    this.filter = { text: '', room: '', showMapped: false, showSecondary: false }
    this.choices = {}
  ...
  run (catalog) {
    ...
    this.rooms = [...new Set(this.devices.flatMap(item => item.device.rooms || []))].sort((a, b) => a.localeCompare(b))
    this.showSelection()
    this.dialog.open()
  }
  ...
  renderToolbar () {
    const toolbar = $('<div>').addClass('nd-toolbar row g-2 align-items-center mb-3')
    ...
    toolbar.append($('<div>').addClass('col-12 col-lg-5').append(search))

    const room = $('<select>').attr({ id: 'nd_room', 'aria-label': this.__('Room') }).addClass('form-select')
    room.append($('<option>').val('').text(this.__('All rooms')))
    this.rooms.forEach(name => room.append($('<option>').val(name).text(name)))
    room.val(this.filter.room).on('change', () => {
      this.filter.room = room.val()
      this.renderList()
    })
    toolbar.append($('<div>').addClass('col-12 col-md-5 col-lg-3').append(room))

    const switches = $('<div>').addClass('col-12 col-md-7 col-lg-4 d-flex flex-wrap gap-3')
  ...
  renderDevice (item, shown) {
    ...
    const details = [device.type].concat(device.rooms || []).concat([device.address])
