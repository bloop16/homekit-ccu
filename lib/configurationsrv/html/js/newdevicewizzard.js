...
import {
  CATEGORIES, REMOTE_SERVICE, SYSTEMS, buildEntries, combinableEntries, combinedName, deviceMappings, entriesSetByGroup, orderWithGroups, channelNumbers, deviceState, hasDefaultName, isMapped, isPerKeyChoice,
...
/** the CCU picture of a device (iconBase: host of the CCU web server, '' for this one), else a symbol */
export function deviceIcon (device, iconBase = '') {
  ...
  if (device.icon) {
    const picture = $('<img>').attr({ src: iconBase + device.icon, alt: '', loading: 'lazy' })
...
export class NewDeviceWizzard extends Wizzard {
  ...
  renderList () {
    this.list.empty()
    let shownDevices = 0
    // CCU groups above their members
    orderWithGroups(this.devices).forEach(({ item, group }) => {
      const shown = visibleEntries(item.device, item.entries, this.filter)
      if (shown.length > 0) {
        shownDevices += 1
        this.list.append(this.renderDevice(item, shown, group))
      }
    })
    if (shownDevices === 0) {
      ...
      .on('click', (e) => {
        ...
        if (chosen.length > 0) {
          ...
          if (device.group === true) {
            entriesSetByGroup(this.devices, device.address).forEach(entry => this.selected.delete(entry.id))
          }
  ...
  renderEntry (entry) {
    ...
    if (entry.byGroup) {
      row.append($('<span>').addClass('badge text-bg-light border flex-shrink-0').text(this.__('set by the group')))
    }
    if (entry.secondary) {
  ...
  showSetup () {
    ...
    this.chosenByDevice().forEach(({ device, entries }) => {
      ...
      box.append(head)
      entries.forEach(entry => this.choiceFor(entry, names[entry.id]))
      const combinable = combinableEntries(entries, this.choices)
      const combined = ((combinable.length > 1) && this.combine.has(device.address)) ? combinable : []
      if (combinable.length > 1) {
        box.append(this.renderCombineSwitch(device, combinable.length))
      }
      entries.forEach(entry => {
        if (combined.includes(entry) && (entry !== combined[0])) {
          box.append(this.renderCombinedRow(entry, combined[0]))
        } else {
          box.append(this.renderSetupRow(entry, this.choices[entry.id], bridges, combined.length > 1 && (entry === combined[0])))
        }
      })
      content.append(box)
    })
  ...
  /** the choice to show several switch outputs of a device as one accessory in Apple Home */
  renderCombineSwitch (device, count) {
    const id = 'nd_combine_' + device.address.replace(/[^A-Za-z0-9]/g, '_')
    const input = $('<input>').attr({ type: 'checkbox', role: 'switch', id }).addClass('form-check-input')
      .prop('checked', this.combine.has(device.address))
      .on('change', () => {
        if (input.prop('checked')) {
          this.combine.add(device.address)
          const first = combinableEntries(this.chosenByDevice().find(item => item.device === device).entries, this.choices)[0]
          this.choices[first.id].name = combinedName(device, this.choices[first.id].name)
        } else {
          this.combine.delete(device.address)
        }
        this.showSetup()
      })
    return $('<div>').addClass('px-3 pt-2')
      .append($('<div>').addClass('form-check form-switch mb-0').append(input)
        .append($('<label>').addClass('form-check-label').attr('for', id).html(this.__('One device in Apple Home with %s switches', count))))
  ...
  /** an output shown as a further switch of the combined accessory */
  renderCombinedRow (entry, primary) {
    const name = hasDefaultName(entry.device, entry.channels[0]) ? this.__('Channel') + ' ' + channelNumbers(entry) : entry.channels[0].name
    return $('<div>').addClass('nd-setup px-3 py-2 small text-body-secondary')
      .append($('<i>').addClass('bi bi-link-45deg me-1').attr('aria-hidden', 'true'))
      .append($('<span>').html(this.__('%s is a switch of %s', name, this.choices[primary.id].name)))
  }

  renderSetupRow (entry, choice, bridges, carriesCombination = false) {
    const row = $('<div>').addClass('nd-setup row g-2 align-items-end px-3 py-2')
    const uid = entry.id.replace(/[^A-Za-z0-9]/g, '_')
    const single = entry.channels.length === 1
    const channelLabel = carriesCombination
      ? this.__('Name of the device')
      : ((entry.kind === 'keys') ? (single ? this.__('Key') : this.__('Keys')) : this.__('Channel')) + ' ' + channelNumbers(entry)
    ...
    service.val(choice.service).on('change', () => {
      ...
      // the choice of combining depends on the switch service
      this.showSetup()
    })
  ...
  async save () {
    ...
    let missing
    this.chosenByDevice().forEach(({ device, entries }) => {
      const combinable = this.combine.has(device.address) ? combinableEntries(entries, this.choices) : []
      const combined = (combinable.length > 1) ? combinable : []
      entries.forEach(entry => {
        ...
        if (combined.includes(entry)) {
          // the first output carries the accessory with the others; name, type and bridge are its own
          if (entry === combined[0]) mappings.push(...deviceMappings(combined, this.choices, choice.bridge, true))
          return
        }
