...
- `FfmpegProcess.js` kapselt `child_process.spawn`, Beenden per SIGTERM und nach 1,5 s SIGKILL, einen Exit-Hook, der beim Prozessende alle Kinder killt, sowie einen Ringpuffer der letzten stderr-Zeilen, der bei unerwartetem Ende in die Warnung wandert (`ffmpegLog.js` redigiert Zugangsdaten in URL und Query-String sowie SRTP-Schlüssel). Ein Start-Timeout gibt es nicht; stattdessen beendet ein RTCP-Watchdog (`rtcpWatchdog.js`, Socket auf dem Video-Rückport, bis zum ersten RTCP-Paket 30 s, danach Stille länger als max(5×rtcp_interval, 10 s), höchstens 60 s) Sessions, deren Viewer verschwunden ist. Snapshots haben einen Timeout (15 s), werden zwischen gleichzeitigen Anfragen geteilt und 5 s gecacht.
- `ffmpegArgs.js` (reine Funktionen `buildSnapshotArgs`, `buildStreamArgs`, `buildReturnAudioSdp`, `buildReturnAudioArgs`): aus `StartStreamRequest`, Session-Info und Einstellungen entsteht das Argument-Array. Keine Seiteneffekte, vollständig testbar.
...
- Audio: Der Controller bietet Opus und AAC-ELD an (Sample-Rate 16 kHz, mono). ffmpegs RTP-Muxer trägt nur einen Stream pro Ausgabe, daher bekommt die Video-Ausgabe `-an -sn -dn` und die Audio-Ausgabe `-vn -sn -dn`. Offen bis zum Gerätetest: die Opus-SDP des Rückkanals nutzt `opus/16000/1` (Sample-Rate wie ausgehandelt); falls Apple Home die RFC-7587-Taktrate sendet, ist `opus/48000/2` nötig. Die Kamera-Tonspur aus `video_source` wird per ffmpeg in den vom Request gewählten Codec transkodiert und als zweiter SRTP-Stream gesendet. Opus nutzt `libopus`, AAC-ELD `libfdk_aac`. Beim Start prüft `FfmpegProcess.probeEncoders()` einmalig per `ffmpeg -encoders`, welche Encoder vorhanden sind; nicht verfügbare Codecs werden nicht angeboten, fehlt beides, wird ohne Audio veröffentlicht und eine Warnung geloggt.
...
Fehlerbehandlung: fehlendes oder nicht ausführbares Binary, fehlender Video-Encoder, leere Quelle oder ein trivialer PIN (HAP-Liste, alter Default `123-45-678`) werden beim Start mit Handlungshinweis geloggt und das Accessory nicht veröffentlicht. Die HomeKit-Identität (`username`) wird aus der Accessory-UUID abgeleitet (`hapIdentity.js`), damit mehrere Klingeln nicht kollidieren; bestehende Klingel-Pairings aus 0.0.x müssen neu gekoppelt werden. ffmpeg-Absturz (Video, Audio oder Rückkanal) beendet nur die betroffene Session (`controller.forceStopStreamingSession`) und alle ihre Prozesse, nie den Server. Ein fehlender Audio-Encoder degradiert auf reines Video statt zu scheitern.
...
- Neu: `test/100_custom_types.js` (Klassen-Factory, Advertiser, publishSingleAccessory), `test/101_time_util.js`, `test/102_ffmpeg_args.js` (Argument-Builder für Snapshot, Start mit und ohne Audio, Opus und AAC-ELD, Rückkanal-SDP, Auflösungen, `copy`), `test/103_ffmpeg_process.js`, `test/104_streaming_delegate.js` (Delegate mit Fake-ffmpeg-Skript unter `test/fixtures/fake-ffmpeg.sh`: Start, Stop, Reconfigure, Absturz, Talkback), `test/105_streaming_options.js`, `test/106_rebuild_class_settings.js`, `test/107_config_restore.js`, `test/108_video_doorbell.js`.
