...
- `FfmpegProcess.js` kapselt `child_process.spawn`, Beenden per SIGTERM und nach 1,5 s SIGKILL, einen Exit-Hook, der beim Prozessende alle Kinder killt, sowie einen Ringpuffer der letzten stderr-Zeilen, der bei unerwartetem Ende in die Warnung wandert (`ffmpegLog.js` redigiert Zugangsdaten und SRTP-Schlüssel). Ein Start-Timeout gibt es nicht; stattdessen beendet ein RTCP-Watchdog (`rtcpWatchdog.js`, Socket auf dem Video-Rückport, Stille länger als max(5×rtcp_interval, 10 s)) Sessions, deren Viewer verschwunden ist. Snapshots haben einen Timeout (15 s), werden zwischen gleichzeitigen Anfragen geteilt und 5 s gecacht.
