...
- `FfmpegProcess.js` kapselt `child_process.spawn`, Beenden per SIGKILL bei STOP oder Fehler, Logging von stderr im Debug-Modus. Ein Start-Timeout gibt es bewusst nicht: ohne Ausgabeanalyse ist „Stream läuft“ nicht messbar; ein abgestürztes ffmpeg beendet die Session, ein hängendes wird durch HomeKits STOP beendet. Nur Snapshots haben einen Timeout (15 s).
