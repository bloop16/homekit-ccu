...
const fs = require('fs')
const cfgFileName = '/etc/config/hm_addons.cfg'
const regex = /(.*?) \{CONFIG_URL (.*?) CONFIG_DESCRIPTION \{de (.*?) en (.*?)\} ID (.*?) CONFIG_NAME (.*?)\} /g
let m
const items = {}
let addItem
...
if (myArgs.length === 2) {
  if (fs.existsSync(myArgs[1])) {
    const buffer = fs.readFileSync(myArgs[1], 'utf8')
    const tmp = JSON.parse(buffer.toString())
    addItem = {
