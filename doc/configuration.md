...
- **A camera picture that is replaced** (a URL or file with the snapshot a camera stores, e.g. an n8n webhook that returns the last snapshot): *Read the picture again after* (default 10 seconds) reads it again when Apple Home asks for the picture, at most that often, in the background; a changed picture is shown from then on. Only what is stored is read, no camera is woken. A URL may contain a user and password (`https://user:password@host/…`, basic auth). Camera pictures fill the tile; 0 reads the picture once.
